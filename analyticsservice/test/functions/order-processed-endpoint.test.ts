...
import { OrderProcessedEndpoint } from "../../src/internal/functions/order-processed-endpoint.js";
import { makeTestStreamContext } from "../support/stream.js";

void test("OrderProcessedEndpoint decodes an event and acknowledges it after the result", async () => {
  const function_ = new OrderProcessedEndpoint();
...
void test("OrderProcessedEndpoint rejects malformed wire data", async () => {
  ...
  await assert.rejects(
    new OrderProcessedEndpoint().consumeMessage(
