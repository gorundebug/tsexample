...
# pnpm/npm normally performs several long retries for one registry URL. The
# outer dependency boundary already retries the complete install operation, so
# keep each individual registry request bounded and single-shot. User-provided
# values remain authoritative.
export NPM_CONFIG_FETCH_RETRIES=${NPM_CONFIG_FETCH_RETRIES:-0}
