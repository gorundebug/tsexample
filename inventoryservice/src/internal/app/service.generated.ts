...
export interface ServiceInfrastructureMakers {
  ...
  inventoryServiceApiGrpcDataSource: (
    context: MessageContext,
    environment: ServiceEnvironment,
    config: Config["named"]["dataConnectors"]["inventoryServiceApi"],
  ) => Promise<GrpcJsDataSource>;
}
...
function defaultInfrastructureMakers(): ServiceInfrastructureMakers {
  return {
    ...
    inventoryServiceApiGrpcDataSource: async (_context, environment, config) =>
      new GrpcJsDataSource(config.id, environment),
  };
}
...
async function initInfrastructure(
  ...
  let inventoryServiceApiGrpcDataSource: GrpcJsDataSource | undefined;
  await Promise.allSettled([
    ...
    invokeMaker(async () => {
      inventoryServiceApiGrpcDataSource = await makers.inventoryServiceApiGrpcDataSource(
  ...
  if (inventoryServiceApiGrpcDataSource === undefined) throw new Error("inventoryServiceApiGrpcDataSource maker returned no data source");
  environment.addDataSource(inventoryServiceApiGrpcDataSource);
  return {
  };
}
