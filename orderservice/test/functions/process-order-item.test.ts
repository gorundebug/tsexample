...
import { ProcessOrderItem } from "../../src/internal/functions/process-order-item.js";
import type { OrderState } from "../../src/internal/types/index.generated.js";
import { makeTestSinkStreamContext } from "../support/stream.js";

void test("ProcessOrderItem sends canonical inventory input and restores caller-owned fields", async () => {
  const function_ = new ProcessOrderItem();
...
void test("ProcessOrderItem turns transport failure into PROCESSING_ERROR", async () => {
  const function_ = new ProcessOrderItem();
  const values: OrderItemResult[] = [];
  const errors: OrderState[] = [];
  const stream = makeTestSinkStreamContext<OrderItem, OrderItemResult, OrderState>(values, errors);
