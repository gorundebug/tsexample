...
void test("OrderProcessedEndpoint writes the canonical keyed Kafka payload", async () => {
...
void test("OrderProcessedEndpoint emits only Kafka delivery failures", async () => {
  const results: Error[] = [];
  let delivery: Promise<void> = Promise.resolve();
