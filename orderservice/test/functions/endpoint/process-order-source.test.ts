...
void test("ProcessOrderSource validates, maps and correlates a complete HTTP order", async () => {
  const response = await runRequest({
    ...
    await new ProcessOrderSource(5_000).consumeMessage(context, collectingStream, state, data, result);
...
void test("ProcessOrderSource rejects an empty order as a client error", async () => {
...
void test("ProcessOrderSource rejects request shapes that violate the generated OpenAPI schema", async () => {
  const response = await runRequest({
...
void test("ProcessOrderSource keeps positive quantity as a business validation rule", async () => {
  const response = await runRequest({
...
async function runRequest(
  ...
  consume: (
    ...
    state: ProcessOrderSourceHandlerState,
    data: HandlerData,
    result: RecordingResult,
  ) => Promise<void>,
): Promise<Response> {
  const endpoint = new ProcessOrderSource(5_000);
...
type ProcessOrderSourceHandlerState = Awaited<ReturnType<ProcessOrderSource["beginRequest"]>>["state"];

class RecordingResult
  implements ResultContext<ProcessOrderSourceHandlerState, ProcessOrderRequest, ProcessOrderResponse, Order, OrderState, Error>
{
  public messageId: string | undefined;
  public callback: ResultCallback<ProcessOrderSourceHandlerState, ProcessOrderRequest, ProcessOrderResponse, Order, OrderState, Error> | undefined;
  public doneCalls = 0;

  public setResultCallback(
    messageId: string,
    callback: ResultCallback<ProcessOrderSourceHandlerState, ProcessOrderRequest, ProcessOrderResponse, Order, OrderState, Error>,
  ): void {
