...
async function initInfrastructure(
  ...
  let orderServiceApiHttpDataSource: NodeHttpDataSource | undefined;
  let inventoryServiceApiGrpcDataSink: GrpcJsDataSink | undefined;
  await Promise.allSettled([
    ...
    invokeMaker(async () => {
      orderServiceApiHttpDataSource = await makers.orderServiceApiHttpDataSource(
        makerContext, environment, config.named.dataConnectors.orderServiceApi,
      );
    }),
    invokeMaker(async () => {
      inventoryServiceApiGrpcDataSink = await makers.inventoryServiceApiGrpcDataSink(
        makerContext, environment, config.named.dataConnectors.inventoryServiceApi,
      );
    }),
  ] as const);
  controller.abort();
  ...
  if (orderServiceApiHttpDataSource === undefined) throw new Error("orderServiceApiHttpDataSource maker returned no data source");
  environment.addDataSource(orderServiceApiHttpDataSource);
  if (inventoryServiceApiGrpcDataSink === undefined) throw new Error("inventoryServiceApiGrpcDataSink maker returned no data sink");
  environment.addDataSink(inventoryServiceApiGrpcDataSink);
  return {
  };
}
