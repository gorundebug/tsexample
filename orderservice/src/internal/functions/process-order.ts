...
export interface ProcessOrderHandlerState {
...
export class ProcessOrder implements HttpSourceEndpointHandler<ProcessOrderHandlerState, ProcessOrderRequest, ProcessOrderResponse, Order, OrderState, Error> {
  ...
  public async beginRequest(context: MessageContext, _stream: StreamContext<Order, OrderState, Error>, data: HandlerData): Promise<{ readonly context: MessageContext; readonly state: ProcessOrderHandlerState }> {
    try {
  ...
  public async consumeMessage(context: MessageContext, stream: StreamContext<Order, OrderState, Error>, state: ProcessOrderHandlerState, data: HandlerData, result: HttpResultContext<ProcessOrderHandlerState, ProcessOrderRequest, ProcessOrderResponse, Order, OrderState, Error>): Promise<void> {
  ...
  public getMessageId(_context: MessageContext, _stream: StreamContext<Order, OrderState, Error>, _state: ProcessOrderHandlerState, value: Readonly<OrderState>): string {
    return value.orderId;
  }

  public endRequest(_context: MessageContext, _stream: StreamContext<Order, OrderState, Error>, error: Error | undefined, _state: ProcessOrderHandlerState, data: HandlerData): void {
...
/** Construct ProcessOrder once while the service graph is initialized. */
export function makeProcessOrder(
  _context: MessageContext,
  _environment: RuntimeEnvironment,
  config: HttpEndpointConfig,
): ProcessOrder {
  const timeout = config.properties["timeout"];
  return new ProcessOrder(typeof timeout === "number" && Number.isFinite(timeout) ? timeout : 5_000);
}
